# Llamado de las librerías en uso
import json
import pandas as pd
...
def guardar_en_excel(dataframe, nombre_excel, hoja=None, columna_inicio=None):
    ...
    :param hoja: El nombre de la hoja en la que se guardarán los datos. Si es
    None, se utiliza la primera hoja por defecto.
    :param columna_inicio: La columna en la que se iniciará la escritura de los
    datos. Si es None, se inicia en la primera columna por defecto.
    """
...
def obtener_lista_encabezado(data):
    ...
    print("==== Finalización obtenido datos del encabezado ====")
...
def obtener_informacion_producto(data):
    ...
    print("==== Finalización obtenido datos de las bases ====")


def obtener_totales(data):
    print("=== Obtenido totales de la transacción ===")
    ...
    print("==== Finalización obtenido totales de la transacción ====")


def obtener_medio_pago(data):
    print("=== Obtenido la información del medio de pago")
    medios = {}
    line_item = data["PosLog"]["Transaction"]["RetailTransaction"]["LineItem"]
    ajuste = False
    for item_interno in line_item:
    ...
    if ajuste:
        ...
        medios["Donacion"] = None
        listado_medios.append(medios)
    print("==== Finalización obtenido la información del medio de pago ====")
...
# Guardar información en el excel
